...
| ✅ Merged | Detalhar pendente/liquidado no drill-down de categoria + tooltip compartilhado | [fork #20](https://github.com/vhsantos26/securo/pull/20), [fork #21](https://github.com/vhsantos26/securo/pull/21) | [#715](https://github.com/securo-finance/securo/issues/715) | [#716](https://github.com/securo-finance/securo/pull/716) |
...
| ✅ Merged | Redesenho do card principal (saldo disponível vs patrimônio líquido) | [fork #11](https://github.com/vhsantos26/securo/pull/11) | [#691](https://github.com/securo-finance/securo/issues/691) | [#785](https://github.com/securo-finance/securo/pull/785) |
...
| ⬜ Só nossa operação (N/A upstream) | Sync de `deploy` com a v0.15.1 do upstream, preservando os ajustes locais (dedupe de saldo de crédito compartilhado, apelido de cartão, categorização de investimentos, renomeação do regime contábil) e renumerando a migration nova do upstream (085 → 092, por colisão de número com a sequência já usada localmente) | — | — | — |
...
| 2026-09-07 | Sync com a v0.15.1 do upstream | Três PRs próprios (hero card do dashboard #785, precedência de regra sobre categoria do provedor #735, drill-down pendente/liquidado/projetado #716) chegaram mergeados no upstream nesta release — o fork já tinha implementações independentes dos três, gerando conflito de reescrita real em `dashboard.tsx`, `transaction-drill-down.tsx` e (sem conflito, texto idêntico) `connection_service.py`. Resolvido combinando as duas versões quando cada lado tinha algo que o outro não tinha (ex.: dashboard.tsx manteve o agrupamento de contas de saldo zero do fork e ganhou o estado de loading/erro e navegação por link do upstream); adotado o upstream quando era superset estrito (drill-down ganhou o bucket "projetado"). Migration nova do upstream (085, exclusão de transação do P&L) colidia de número com a sequência já usada localmente para outras 8 migrations (invoices, cartão vinculado, etc., que já existiam idênticas sob outros números por causa de um sync anterior squashed) — renumerada para 092, cadeia validada com `check_migration_chain.py`. Os 12 idiomas existentes ganharam merge automatizado por script (JSON deep-merge preferindo o valor local em conflito, união de chaves), preservando as 6 chaves customizadas do hero card; os 3 idiomas novos do upstream (el/hi/ja) entraram limpos mas sem as ~51 chaves específicas do fork (workspace.integrations, apelido de cartão, categorização de investimentos, regime contábil) — preenchidas com o texto em inglês como placeholder até termos tradução real, para o teste de paridade de chaves (`i18n.test.ts`) passar. Suíte completa verde: 3498 passed / 46 skipped no backend, 702/702 no frontend, typecheck e build de produção OK. | Claude |
